		...
		if (email.isEmpty()) {
			...
			for (int i = 0; i < existingUserList.size(); i++) {
				...
				if (existingUserList.get(i).getEmail().toString().equals(email)) {
					...
					writer.println("window.location.href = 'http://localhost:8091/Bookee-Project/resetPassword.jsp';");
					writer.println("</script>");
					break;
				}else {
					writer.println("<script type=\"text/javascript\">");
				    writer.println("alert('The e-mail provided is not linked to any account');");
				    writer.println("window.location.href = 'http://localhost:8091/Bookee-Project/forgetPassword.jsp';");
				    writer.println("</script>");
